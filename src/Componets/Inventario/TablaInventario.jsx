    ...
    const submitCreate = async () => {
        ...
        if (activeTab === 'productos') {
            if (!createData.codigo_producto?.trim()) {
                errors.codigo_producto = "El código es obligatorio";
            } else if (createData.codigo_producto.trim().length > 20) {
                errors.codigo_producto = "No debe superar los 20 caracteres";
            }
            ...
            if (!createData.id_producto) {
                errors.id_producto = "El producto es obligatorio";
            }
            if (!createData.tipo_movimiento) {
                errors.tipo_movimiento = "El tipo de movimiento es obligatorio";
            }
            if (createData.cantidad === undefined || createData.cantidad === null || String(createData.cantidad).trim() === '') {
                errors.cantidad = "La cantidad es obligatoria";
            } else {
                const cantidadNum = Number(createData.cantidad);
                        ...
                        <div className="flex flex-col gap-4">
                            ...
                            {activeTab === 'productos' && (
                                <>
                                    ...
                                    <TextArea label="Descripción" name="descripcion" onChange={handleCreateChange} value={createData.descripcion || ''} error={createErrors.descripcion} />
                                    <Select label="Categoría" name="id_categoria" value={createData.id_categoria || ''} onChange={handleCreateChange} error={createErrors.id_categoria}>
                                    ...
                                    <Input label="Precio Unitario" type="number" step="0.01" name="precio_unitario" onChange={handleCreateChange} value={createData.precio_unitario || ''} error={createErrors.precio_unitario} />
                                    <Select label="Gerencia" name="id_gerencia" value={createData.id_gerencia || ''} onChange={handleCreateChange} error={createErrors.id_gerencia}>
                                        <option value="">Seleccione...</option>
                            ...
                            {activeTab === 'movimientos' && (
                                <>
                                    ...
                                    <Select label="Producto" name="id_producto" value={createData.id_producto || ''} onChange={handleCreateChange} error={createErrors.id_producto}>
                                        <option value="">Seleccione...</option>
                                        {productResults.map(p => <option key={p.id_producto} value={p.id_producto}>{p.nombre_producto}</option>)}
                                    </Select>
                                    <Select label="Tipo Movimiento" name="tipo_movimiento" value={createData.tipo_movimiento || ''} onChange={handleCreateChange} error={createErrors.tipo_movimiento}>
                                        <option value="">Seleccione...</option>
                                        <option value="Entrada">Entrada</option>
                                        <option value="Salida">Salida</option>
                                        <option value="Ajuste">Ajuste</option>
                                    </Select>
                                    <Input label="Cantidad" type="number" name="cantidad" onChange={handleCreateChange} value={createData.cantidad || ''} error={createErrors.cantidad} />
                                    <TextArea label="Motivo (Opcional)" name="motivo" onChange={handleCreateChange} value={createData.motivo || ''} error={createErrors.motivo} />
                                </>
                            )}
